...
BlockController.prototype.getBlockByHash = function (hash, next) {
    ...
    if (blockCached) {
        ...
        , function (callback) {
            ...
            return self.node.services.bitcoind.getDetailedTransaction(txHash, function (err, trx) {
...
BlockController.prototype.transformBlock = function (block, info, reward, transaction) {
    ...
    if (block.header.isProofOfStake()) {
        poolName = transaction.inputs[0].address;
    } else {
        poolName = transaction.outputs[0].address;
    }
...
BlockController.prototype._getBlockSummary = function (hash, moreTimestamp, next) {
    ...
    if (summaryCache) {
        ...
        return async.waterfall([function (callback) {
            ...
            switch (block.flags) {
                case bitcore.Block.PROOF_OF_STAKE:
                    summary.poolInfo.poolName = transaction.inputs[0].address;
                    break;
                case bitcore.Block.PROOF_OF_WORK:
                    summary.poolInfo.poolName = transaction.outputs[0].address;
                    break;
            }
