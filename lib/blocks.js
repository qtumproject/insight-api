...
BlockController.prototype.transformBlock = function (block, info, reward) {
    ...
    return {
        ...
        poolInfo: this.getPoolInfo(bitcore.Block(block))
    };
};
        ...
        return self.node.services.bitcoind.getRawBlock(hash, function (err, blockBuffer) {
            ...
            self.node.services.bitcoind.getBlockHeader(hash, function (err, blockHeader) {
                ...
                var summary = {
                    ...
                    time: header.time,
                    txlength: txlength,
                    poolInfo: self.getPoolInfo(bitcore.Block(blockBuffer)),
...
BlockController.prototype.getPoolInfo = function (block) {
    ...
    if (block.header && block.header.isProofOfStake() && block.transactions[1] && block.transactions[1].inputs[0] && block.transactions[1].inputs[0].script) {
        var address = block.transactions[1].inputs[0].script.toAddress(this.node.network);
