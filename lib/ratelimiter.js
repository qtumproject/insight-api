 ...
 * @param {Number} options.banWhitelistInterval
 * @param {Number} options.banBlacklistInterval
 * @param {Number} options.banInterval
 */
...
RateLimiter.prototype.exceeded = function(client) {
  if (this.config[client.type].totalRequests === -1) {
    ...
    if (client.visits > this.config[client.type].totalRequests) {
...
RateLimiter.prototype.addClient = function(name) {
  ...
  var banInterval = this.config[client.type].banInterval;

  setTimeout(function() {

    if (self.clients[name] && !self.clients[name]['isBanned']) {
