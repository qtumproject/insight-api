        ...
        return async.eachOfSeries(createVouts, function (vout, voutCreateIdx, callback) {
            if (vout.scriptPubKey && vout.scriptPubKey.type === 'create' && ContractsHelper.isErc20Contract(vout.scriptPubKey.hex)) {
                ...
                return async.waterfall([function (callback) {
                    ...
                    return self.node.callContract(contractAddress, functionHashes['balanceOf(address)'] + '000000000000000000000000' + creatorAddress, {}, function(err, data) {
                        ...
                        if (data && data.executionResult) {
                            try {
                                ...
                                if (decodedBalance.valueOf() > 0) {

                                    var erc20BalanceObject = {
                                        address_eth: '0x' + creatorAddress,
                                        address: ContractsHelper.getBitAddressFromContractAddress(creatorAddress, self.node.network.pubkeyhash.toString(16)),
                                        contract_address_base: ContractsHelper.getBitAddressFromContractAddress(contractAddress, self.node.network.pubkeyhash.toString(16)),
                                        contract_address: contractAddress,
                                        amount_hex: decodedBalance.toString(16),
                                        amount: decodedBalance.toString(10),
                                    }

                                    return self.erc20BalanceRepository.createOrUpdateBalance(erc20BalanceObject, function(err, row) {
                                        if (err) {
                                            self.common.log.error('[ERC20WATCHER] error createOrUpdateBalance', err);
                                            return callback(err);
                                        }

                                        self.common.log.info('[ERC20WATCHER] createOrUpdateBalance', row);
